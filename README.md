...
- [ ] Deal Intake 文件型材料解析（上传 PDF/Word/Excel BP → 抽取文本喂入分析流；当前自然语言触发以消息正文为材料）；系统主动推送触发流；deal_sourcing 搜寻流 mine_signals 也接入企查查 company_lookup（当前仅分析流 enrich_external 用工商源）;前端 deal_ref 块渲染与项目工作台详情页
