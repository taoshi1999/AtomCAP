...
- [ ] **审阅接受后版本化 Preference**：accept 后把 suggested_changes 应用到 `InvestmentPreference.learned_preference`，经服务层创建新 active 版本（复用 preferences 写路径）、旧版置否、写 `preference.updated`，PreferenceAdvice 标 `applied`；保留版本溯源（base_preference_version → 新 version）
- [ ] **三 Agent 读 learned_preference 反哺**：赛道前瞻 / 项目获取 / Pre-DD 在生成时读取机构当前 `learned_preference` 权重表与 `risk_boundary`，影响赛道推荐排序、候选 fit_score 与风险初筛
