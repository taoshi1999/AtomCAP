...
/** 已落库项目：上传选项仅反映数据库中的材料行 */
function backendMaterialsToUploadFolders(
  approved: { id: string; name: string; format: string; size: string; description: string }[],
): MockFolder[] {
  if (approved.length === 0) return []
  return [
    {
      id: "backend-upload-root",
      name: "项目材料（已入库）",
      files: approved.map((m) => ({
        id: m.id,
        name: /\.[a-z0-9]+$/i.test(m.name) ? m.name : `${m.name}.${m.format}`,
        format: m.format,
        size: m.size,
...
export function ProjectMaterials({
  ...
  /** `/projects/[id]` 已入库项目：列表与上传选项均来自接口，不落本地演示数据 */
  const isPersistedBackendProject = Boolean(projectId && !projectId.startsWith("new-project-"))
  ...
  const uploadFolderSource = useMemo((): MockFolder[] => {
    if (!isPersistedBackendProject) return mockLocalFolders
    return backendMaterialsToUploadFolders(approvedItems)
  }, [isPersistedBackendProject, approvedItems])

  const appendStaticDemoMaterials = Boolean(
    isNewProject && inheritedFromParent && !isPersistedBackendProject,
  )

  const displayItems = useMemo(
    () => [...approvedItems, ...(appendStaticDemoMaterials ? materials : [])],
    [approvedItems, appendStaticDemoMaterials],
  )

  const dialogHypotheses = useMemo(() => {
    if (projectHypotheses && projectHypotheses.length > 0) return projectHypotheses
    return isPersistedBackendProject ? [] : FALLBACK_HYPOTHESES
  }, [projectHypotheses, isPersistedBackendProject])

  const dialogTerms = useMemo(() => {
    if (projectTerms && projectTerms.length > 0) return projectTerms
    return isPersistedBackendProject ? [] : FALLBACK_TERMS
  }, [projectTerms, isPersistedBackendProject])
  ...
  if (materialPrefill && !prefillApplied) {
    ...
    setCurrentFolderId(isPersistedBackendProject ? null : folderId)
    setSelectedFiles(new Set(isPersistedBackendProject ? [] : getMatchingFileIds(materialPrefill.category)))
    setUploadState("idle")
    setUploadProgress(0)
  ...
  function handleUpload() {
    ...
    setTimeout(() => {
      const allFiles = uploadFolderSource.flatMap((f) => f.files)
      const selectedFileData = allFiles.filter((f) => selectedFiles.has(f.id))
      const rawTitle = dialogMaterialTitle || selectedFileData[0]?.name || "上传材料"
      const primaryName = rawTitle.replace(/\.[^.]+$/, "")

      if (onCreatePendingProjectMaterial && projectId) {
        const first = selectedFileData[0]
        const pendingProject: PendingProjectMaterial = {
          id: `pending-project-material-${Date.now()}`,
          projectId,
          projectName: project?.name || "当前项目",
          material: {
            name: primaryName,
            format: first?.format ?? "PDF",
            size: first?.size ?? "—",
            category: dialogMaterialCategory || "通用材料",
            description: formDescription || first?.description || "",
            collectReason: formDescription || "从项目已入库材料中选择并提交",
          },
          changeId: `CR-PM-${Date.now().toString().slice(-6)}`,
          changeName: `上传项目材料: ${primaryName}`,
          changeType: "collect",
        ...
        void Promise.resolve(onCreatePendingProjectMaterial(pendingProject)).finally(() => {
          setIsUploadOpen(false)
          setUploadState("idle")
          setUploadProgress(0)
          setSelectedFiles(new Set())
  ...
  const currentFolder = uploadFolderSource.find((f) => f.id === currentFolderId) ?? null
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1.5 rounded-md bg-[#F9FAFB] border border-[#E5E7EB] px-3 py-2 text-xs text-[#6B7280]">
                  ...
                  <span>{isPersistedBackendProject ? "项目材料库（后端）" : "本机文档"}</span>
                  {currentFolder && (
                    <>
                ...
                <div className="rounded-lg border border-[#E5E7EB] bg-white overflow-hidden">
                  ...
                  <div className="max-h-[220px] overflow-y-auto divide-y divide-[#F3F4F6]">
                    ...
                    ) : uploadFolderSource.length === 0 ? (
                      <div className="px-4 py-8 text-center text-xs leading-relaxed text-[#9CA3AF]">
                        {isPersistedBackendProject
                          ? "当前没有在库材料，无法从库中选择。请先在项目中创建材料或通过管理端写入后再试。"
                          : "暂无可用文件夹。"}
                      </div>
                    ) : (
                      uploadFolderSource.map((folder) => {
