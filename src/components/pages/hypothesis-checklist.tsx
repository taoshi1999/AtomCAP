...
import { parseHypothesisBodySections } from "@/src/lib/minimax-body-parse"
import { classifyHypothesisBodySection } from "@/src/lib/minimax-section-classify"
...
export interface HypothesisTableItem {
  ...
  name: string       // 假设名称
  /** 案例/PDF 全文（论据论证等），来自数据库 body 字段 */
  body?: string
  owner: string      // 负责人
  createdAt: string  // 创建时间
  updatedAt: string  // 更改时间
  status: "verified" | "pending" | "risky"
  /** 已落库：getOverview 按价值/风险点 id 注入的评论 */
  pointComments?: Record<string, { id: string; author: string; content: string; time: string }[]>
}
...
function mergeDetailPointComments(
  detail: HypothesisDetail,
  pointComments?: Record<string, { id: string; author: string; content: string; time: string }[]>,
): HypothesisDetail {
  if (!pointComments || Object.keys(pointComments).length === 0) return detail
  const inject = <T extends ValuePoint | RiskPoint>(pts: T[]): T[] =>
    pts.map((p) => ({
      ...p,
      comments: [
        ...(p.comments ?? []),
        ...(pointComments[p.id] ?? []).map((c) => ({
          author: c.author,
          content: c.content,
          time: c.time,
        })),
  ...
  return {
    ...detail,
    valuePoints: inject(detail.valuePoints ?? []),
    riskPoints: inject(detail.riskPoints ?? []),
  }
}
...
type DisplayBodySection = BodySection & {
  /** support：论据/论证/原文章节等依据链，与价值判断正文区分，避免「减值风险」等词误判 */
  kind: "value" | "risk" | "opinion" | "support" | "other"
  displayTitle: string
}

function buildDisplaySections(body?: string): DisplayBodySection[] {
  return parseHypothesisBodySections(body).map((section) => {
    const kind = classifyHypothesisBodySection(section)
    return {
      ...section,
      kind,
...
/** 未点击标签时：优先选首个有内容的分段，避免「仅 other/详情」的正文默认落在「价值」导致预览空白（种子与 DB 行式正文常见） */
function defaultHypothesisBodyFocus(
  explicit: "value" | "risk" | "opinion" | "support" | "extra" | undefined,
  buckets: {
    value: DisplayBodySection[]
    risk: DisplayBodySection[]
    opinion: DisplayBodySection[]
    support: DisplayBodySection[]
    extra: DisplayBodySection[]
  },
...
/** 与案例摘录章节顺序一致：团队 → 财务 → 技术 → 商业化 → 市场 */
function getDirectionOrder(direction: string): number {
  ...
  if (direction.includes("团队")) return 10
  if (direction.includes("财务")) return 20
  if (direction.includes("技术")) return 30
  if (direction.includes("商业化")) return 35
  if (direction.includes("市场")) return 40
  return 99
}

function buildFallbackHypothesisDetail(item: HypothesisTableItem): HypothesisDetail {
  const today = new Date().toISOString().split("T")[0]!
  const sections = buildDisplaySections(item.body)
  ...
  const otherSections = sections.filter((s) => s.kind === "other")
  const baseSection: DisplayBodySection = sections[0] ?? {
    title: "详情",
    displayTitle: "详情",
    content: item.body?.trim() || "待补充",
    kind: "other",
  }

  const toPoint = (prefix: "vp" | "rp", sec: DisplayBodySection, idx: number): ValuePoint | RiskPoint => ({
    id: `${item.id}-${prefix}-${idx + 1}`,
    title: sec.displayTitle,
    evidence: { description: sec.content, files: [] },
    analysis: {
      content: sec.content,
      creator: { name: item.owner || "未指定", role: "负责人" },
      reviewers: [],
      createdAt: today,
    },
    comments: [],
  ...
  const verificationContent = [...opinionSections, ...supportSections, ...otherSections]
  ...
  return {
    id: item.id,
    title: item.name,
    qaId: `QA-${item.id.slice(0, 8).toUpperCase()}`,
    createdAt: item.createdAt,
    updatedAt: item.updatedAt,
    status: item.status,
    creator: { name: item.owner || "未指定", role: "负责人" },
    valuePoints: (valueSections.length > 0 ? valueSections : [{ ...baseSection, kind: "value" as const, displayTitle: "价值1" }]).map((s, idx) => toPoint("vp", s, idx) as ValuePoint),
    riskPoints: (riskSections.length > 0 ? riskSections : [{ ...baseSection, kind: "risk" as const, displayTitle: "风险1" }]).map((s, idx) => toPoint("rp", s, idx) as RiskPoint),
    committeeDecision: {
      conclusion: "",
      status: "pending",
      content: "",
    ...
    verification: {
      conclusion: "",
      status: "pending",
      content: verificationContent || item.body || "",
      creator: { name: item.owner || "未指定", role: "负责人" },
      reviewers: [],
      createdAt: today,
      comments: [],
...
interface HypothesisChecklistProps {
  ...
  /** 已落库项目：价值/风险点评论写入 DB */
  onAddHypothesisPointComment?: (input: {
    projectId: string
    hypothesisId: string
...
export function HypothesisChecklist({
  ...
  // 已落库项目：仅展示 props 传入的清单（来自 getWorkspace）；未拉到数据前不要用内置演示表顶替。
  const sourceData =
    inheritedHypotheses != null
      ? inheritedHypotheses
  ...
  const directionTabs = Array.from(new Set(sourceData.map((item) => item.direction)))
    .sort((a, b) => getDirectionOrder(a) - getDirectionOrder(b))
  ...
  const filteredData = sourceData
    ...
    .sort((a, b) => getDirectionOrder(a.direction) - getDirectionOrder(b.direction))

  // 详情优先 extraDetails；若结构化字段为空则用 body 回填
  const selectedDetail = selectedId
    ? (() => {
      const item = sourceData.find((h) => h.id === selectedId)
      if (!item) return null
      const fallback = buildFallbackHypothesisDetail(item)
      const existing = extraDetails?.[selectedId] ?? hypothesisDetails[selectedId]
      const hasText = (text?: string) => (text?.trim().length ?? 0) > 0
      let base: HypothesisDetail
      if (!existing) {
        base = fallback
      } else {
        base = {
          ...existing,
          valuePoints:
            (existing.valuePoints?.length ?? 0) > 0 ? existing.valuePoints : fallback.valuePoints,
          riskPoints:
            (existing.riskPoints?.length ?? 0) > 0 ? existing.riskPoints : fallback.riskPoints,
          verification: hasText(existing.verification?.content)
            ? existing.verification
            : fallback.verification,
        }
      }
      return mergeDetailPointComments(base, item.pointComments)
    })()
    : null
  ...
  async function handleSendComment(pointId: string, kind: "value" | "risk") {
    ...
    if (
      ...
      try {
        await onAddHypothesisPointComment({
          projectId,
        ...
        <div className="px-6 py-6">
          ...
          <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 mb-6">
            ...
            <p className="text-sm text-[#6B7280] mb-3">
              ID: {selectedDetail.qaId} | 创建时间: {formatDisplayDateTime(selectedDetail.createdAt)} | 更新时间: {formatDisplayDateTime(selectedDetail.updatedAt)}
            </p>
              ...
              <div key={vp.id} className="bg-white rounded-xl border border-[#E5E7EB] mb-4 overflow-hidden">
                <div className="border-l-4 border-[#22C55E] p-5">
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-[#111827] mb-3 whitespace-pre-wrap">{vp.evidence.description}</p>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="p-3 bg-[#F9FAFB] rounded-lg">
                      <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{vp.analysis.content}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") void handleSendComment(vp.id, "value")
                        }}
                      ...
                      />
                      <button
                        onClick={() => void handleSendComment(vp.id, "value")}
            ...
            {[...selectedDetail.riskPoints, ...(localRiskPoints[selectedId ?? ""] || [])].map((rp) => (
              <div key={rp.id} className="bg-white rounded-xl border border-[#E5E7EB] mb-4 overflow-hidden">
                <div className="border-l-4 border-[#EF4444] p-5">
                  ...
                  <div className="mb-4">
                    <p className="text-xs text-[#6B7280] mb-1">论据支持</p>
                    <p className="text-sm text-[#111827] mb-3 whitespace-pre-wrap">{rp.evidence.description}</p>
                  ...
                  <div className="mb-4">
                    <p className="text-xs text-[#6B7280] mb-1">论证分析</p>
                    <div className="p-3 bg-[#F9FAFB] rounded-lg">
                      <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{rp.analysis.content}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") void handleSendComment(rp.id, "risk")
                        }}
                        className="flex-1 text-sm border border-[#E5E7EB] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
                      />
                      <button
                        onClick={() => void handleSendComment(rp.id, "risk")}
                        className="p-2 text-[#2563EB] hover:bg-[#EFF6FF] rounded-lg transition-colors"
                      >
                        <Send className="h-4 w-4" />
                      </button>
                ...
                {selectedDetail.committeeDecision.content ? (
                  <>
                    ...
                    <div className="p-3 bg-[#F9FAFB] rounded-lg mb-4">
                      <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{selectedDetail.committeeDecision.content}</p>
                    </div>
                  </>
                ) : (
              ...
              <div className="border-l-4 border-[#8B5CF6] p-5">
                {selectedDetail.verification.content ? (
                  <>
                    ...
                    <div className="p-3 bg-[#F9FAFB] rounded-lg mb-4">
                      <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{selectedDetail.verification.content}</p>
                    </div>
                  </>
                ) : (
                    ...
                    {item.body ? (
                      <div className="mt-2">
                        {(() => {
                          ...
                          const focus = defaultHypothesisBodyFocus(bodyFocus[item.id], {
                            value: valueSections,
                            risk: riskSections,
                          ...
                          return (
                            <div className="rounded border border-[#E5E7EB] bg-[#FAFAFA]">
                              {/* 禁止使用 button 包裹 button，否则触发 hydration 报错 */}
                              <div className="flex w-full items-center justify-between gap-2 px-2 py-1.5 hover:bg-[#F3F4F6]">
                                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
                                  <button
                                    type="button"
                                    onClick={() => setExpandedBodies((prev) => ({ ...prev, [item.id]: !isExpanded }))}
                                    className="text-left text-[11px] text-[#4B5563] hover:text-[#111827]"
                                  >
                                    点击{isExpanded ? "收起" : "展开"}假设细则
                                  </button>
                                  <button
                                    type="button"
                                ...
                                <button
                                  type="button"
                                  onClick={() => setExpandedBodies((prev) => ({ ...prev, [item.id]: !isExpanded }))}
                                  className="shrink-0 rounded p-0.5 text-[#6B7280] hover:bg-[#E5E7EB]"
                                  aria-expanded={isExpanded}
                                  aria-label={isExpanded ? "收起假设细则" : "展开假设细则"}
                                >
                                  {isExpanded ? (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
              <div className="flex items-center justify-between border-b border-[#E5E7EB] px-6 py-4">
                <h3 className="text-base font-semibold text-[#111827]">新建假设</h3>
              ...
              <div className="space-y-4 px-6 py-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-[#374151]">假设方向</label>
                    <Input value={createForm.direction} onChange={(e) => setCreateForm((prev) => ({ ...prev, direction: e.target.value }))} placeholder="例如：一、团队与组织能力" />
                  </div>
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-[#374151]">假设类别</label>
                    <Input value={createForm.category} onChange={(e) => setCreateForm((prev) => ({ ...prev, category: e.target.value }))} placeholder="例如：创始人背景" />
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#374151]">假设名称</label>
                  <Input value={createForm.name} onChange={(e) => setCreateForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="请输入假设名称" />
                </div>
                <div>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-[#E5E7EB] px-6 py-4">
                ...
                <Button onClick={handleCreateHypothesisSubmit} disabled={isCreating}>
                  {isCreating ? "创建中..." : "确认创建"}
                </Button>
              </div>
            </div>
