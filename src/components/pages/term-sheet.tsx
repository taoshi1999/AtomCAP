...
import { parseTermBodyLineSections } from "@/src/lib/minimax-body-parse"
import { classifyTermBodySection } from "@/src/lib/minimax-section-classify"
...
export interface TermTableItem {
  ...
  name: string       // 条款名称
  /** 条款细则全文，来自数据库 body 字段 */
  body?: string
  owner: string      // 负责人
  createdAt: string  // 创建时间
  updatedAt: string  // 更改时间
  status: "approved" | "pending" | "rejected"
  /** 已落库：getOverview 按区块 sectionKey 注入的评论 */
  sectionComments?: Record<string, { id: string; author: string; content: string; time: string }[]>
}

/** 与 `project.addTermSectionComment` / Prisma 区块一致 */
export type TermSectionCommentKey =
  | "ourDemand"
  | "ourBasis"
  | "bilateralConflict"
  | "ourBottomLine"
...
function mergeTermSectionCommentsIntoDetail(
  detail: TermDetail,
  sectionComments?: Record<string, { id: string; author: string; content: string; time: string }[]>,
): TermDetail {
  if (!sectionComments || Object.keys(sectionComments).length === 0) return detail
  const inject = (
    existing: { author: string; content: string; time: string }[],
    key: TermSectionCommentKey,
  ) => [
    ...existing,
    ...(sectionComments[key] ?? []).map((c) => ({
      author: c.author,
      content: c.content,
      time: c.time,
    })),
  ...
  return {
    ...detail,
    ourDemand: { ...detail.ourDemand, comments: inject(detail.ourDemand.comments, "ourDemand") },
    ourBasis: { ...detail.ourBasis, comments: inject(detail.ourBasis.comments, "ourBasis") },
    bilateralConflict: {
      ...detail.bilateralConflict,
      comments: inject(detail.bilateralConflict.comments, "bilateralConflict"),
    },
    ourBottomLine: {
      ...detail.ourBottomLine,
      comments: inject(detail.ourBottomLine.comments, "ourBottomLine"),
    },
    compromiseSpace: {
      ...detail.compromiseSpace,
      comments: inject(detail.compromiseSpace.comments, "compromiseSpace"),
    },
    negotiationResult: {
      ...detail.negotiationResult,
      comments: inject(detail.negotiationResult.comments, "negotiationResult"),
    },
    implementationStatus: {
      ...detail.implementationStatus,
      comments: inject(detail.implementationStatus.comments, "implementationStatus"),
    },
  }
}
...
type TermBodySection = {
  title: string
  content: string
}

type DisplayTermBodySection = TermBodySection & {
  kind: "value" | "risk" | "opinion" | "support" | "other"
  displayTitle: string
}

function buildDisplayTermSections(body?: string): DisplayTermBodySection[] {
  return parseTermBodyLineSections(body).map((section) => {
    const kind = classifyTermBodySection(section)
    return {
      ...section,
      kind,
...
function defaultTermBodyFocus(
  explicit: "value" | "risk" | "opinion" | "support" | "extra" | undefined,
  buckets: {
    value: DisplayTermBodySection[]
    risk: DisplayTermBodySection[]
    opinion: DisplayTermBodySection[]
    support: DisplayTermBodySection[]
    extra: DisplayTermBodySection[]
  },
): "value" | "risk" | "opinion" | "support" | "extra" {
  if (explicit) return explicit
  if (buckets.value.length > 0) return "value"
  if (buckets.risk.length > 0) return "risk"
  if (buckets.opinion.length > 0) return "opinion"
  if (buckets.support.length > 0) return "support"
  if (buckets.extra.length > 0) return "extra"
  return "value"
}

function getTermDirectionOrder(direction: string): number {
  if (direction.startsWith("一、")) return 1
  if (direction.startsWith("二、")) return 2
  if (direction.startsWith("三、")) return 3
  if (direction.startsWith("四、")) return 4
  if (direction.startsWith("五、")) return 5
  if (direction.includes("资本") || direction.includes("下行") || direction.includes("经济")) return 1
  if (direction.includes("控制权") || direction.includes("治理")) return 2
  if (direction.includes("激励") || direction.includes("长期")) return 3
  if (direction.includes("退出")) return 4
  if (direction.includes("信息权") || direction.includes("保护")) return 5
  return 99
}

function trimTitleKey(s: string): string {
  return s.replace(/\s+/g, "").trim()
}

function buildFallbackTermDetail(item: TermTableItem): TermDetail {
  const today = new Date().toISOString().split("T")[0]!
  const sections = buildDisplayTermSections(item.body)
  const valueContent = sections
    .filter((s) => s.kind === "value")
    .map((s) => `【${s.displayTitle}】\n${s.content}`)
    .join("\n\n")
  const riskSections = sections.filter((s) => s.kind === "risk")
  const conflictRiskSections = riskSections.filter((s) => !trimTitleKey(s.displayTitle).startsWith("我方底线"))
  const bottomLineRiskSections = riskSections.filter((s) =>
    trimTitleKey(s.displayTitle).startsWith("我方底线"),
  )
  const fmt = (secs: DisplayTermBodySection[]) =>
    secs.map((s) => `【${s.displayTitle}】\n${s.content}`).join("\n\n")
  const riskContent = fmt(conflictRiskSections)
  const bottomLineContent = fmt(bottomLineRiskSections)
  const opinionContent = sections
    .filter((s) => s.kind === "opinion")
    .map((s) => `【${s.displayTitle}】\n${s.content}`)
    .join("\n\n")
  const supportContent = sections
    .filter((s) => s.kind === "support")
    .map((s) => `【${s.displayTitle}】\n${s.content}`)
    .join("\n\n")
  ...
  const singleUnstructured = sections.length === 1 && sections[0]?.kind === "other"
  const ourDemandContent = valueContent || (singleUnstructured ? item.body?.trim() ?? "" : "")

  const creator = { name: item.owner || "未指定", role: "负责人" }
  const sectionBase = {
    creator,
    reviewers: [],
    createdAt: today,
  ...
  return {
    id: item.id,
    title: item.name,
    termId: `TM-${item.id.slice(0, 8).toUpperCase()}`,
    createdAt: item.createdAt,
    updatedAt: item.updatedAt,
    status: item.status,
    creator,
    ourDemand: { content: ourDemandContent, ...sectionBase },
    ourBasis: { content: supportContent || "", ...sectionBase },
    bilateralConflict: { content: riskContent || "", ...sectionBase },
    ourBottomLine: { content: bottomLineContent || "", ...sectionBase },
    compromiseSpace: { content: [opinionContent, otherContent].filter(Boolean).join("\n\n"), ...sectionBase },
    negotiationResult: { conclusion: "", status: "partial", content: "", creator, reviewers: [], createdAt: today, comments: [] },
    implementationStatus: { status: "not-started", content: "", creator, reviewers: [], createdAt: today, comments: [] },
  }
}
...
interface TermSheetProps {
  ...
  /** 已落库：条款分区评论写入 DB */
  onAddTermSectionComment?: (input: {
    projectId: string
    termId: string
    sectionKey: TermSectionCommentKey
    content: string
  }) => void | Promise<void>
}
...
export function TermSheet({
  ...
  inheritedTerms,
  extraDetails,
  onCreateNegotiationDecision,
  onCreateImplementationStatus,
  onCreateTerm,
  onAddTermSectionComment,
}: TermSheetProps) {
  ...
  const [bodyFocus, setBodyFocus] = useState<Record<string, "value" | "risk" | "opinion" | "support" | "extra">>({})
  ...
  const [createForm, setCreateForm] = useState({ direction: "", category: "", name: "", body: "" })
  const [isCreating, setIsCreating] = useState(false)
  const [createError, setCreateError] = useState("")
  const [termSectionCommentInputs, setTermSectionCommentInputs] = useState<Record<string, string>>({})
  /** 未落库项目：分区评论仅存前端 */
  const [extraSectionComments, setExtraSectionComments] = useState<
  ...
  function openCreateDialog() {
    const defaultDirection =
      activeDirection !== "all"
        ? activeDirection
        : directionTabs[0] || "一、资本安全与下行保护"
    setCreateForm({ direction: defaultDirection, category: "", name: "", body: "" })
    setCreateError("")
    setShowCreateDialog(true)
  }

  async function handleCreateTermSubmit() {
    if (!createForm.direction.trim() || !createForm.category.trim() || !createForm.name.trim()) {
      setCreateError("请填写方向、类别和名称")
      return
    }
    if (!onCreateTerm) {
      setCreateError("当前项目暂不支持直接新建，请先走工作流创建")
      return
    }
    try {
      setIsCreating(true)
      ...
      setCreateError(e instanceof Error ? e.message : "新建失败，请稍后重试")
    } finally {
      setIsCreating(false)
    }
  }

  const isPersistedProject = Boolean(projectId && !projectId.startsWith("new-project-"))
  const sourceData =
    inheritedTerms != null
      ? inheritedTerms
      : isNewProject && project?.strategyId === "1"
        ...
        : isPersistedProject
          ? []
          : termTableData
  const directionTabs = Array.from(new Set(sourceData.map((item) => item.direction)))
    .sort((a, b) => getTermDirectionOrder(a) - getTermDirectionOrder(b))

  // Filter data
  const filteredData = sourceData
    .filter((item) => activeDirection === "all" || item.direction === activeDirection)
    .filter((item) => {
    ...
    .sort((a, b) => getTermDirectionOrder(a.direction) - getTermDirectionOrder(b.direction))

  // 详情优先 extraDetails；若结构化字段为空则用 body 回填，保证 DB 记录展示一致
  const selectedDetail = selectedId
    ? (() => {
      const item = sourceData.find((t) => t.id === selectedId)
      if (!item) return null
      const fallback = buildFallbackTermDetail(item)
      const existing = extraDetails?.[selectedId] ?? termDetails[selectedId]
      const hasText = (text?: string) => (text?.trim().length ?? 0) > 0
      let base: TermDetail
      if (!existing) {
        base = fallback
      } else {
        base = {
          ...existing,
          ourDemand: hasText(existing.ourDemand?.content) ? existing.ourDemand : fallback.ourDemand,
          ourBasis: hasText(existing.ourBasis?.content) ? existing.ourBasis : fallback.ourBasis,
          bilateralConflict: hasText(existing.bilateralConflict?.content)
            ? existing.bilateralConflict
            : fallback.bilateralConflict,
          ourBottomLine: hasText(existing.ourBottomLine?.content)
            ? existing.ourBottomLine
            : fallback.ourBottomLine,
          compromiseSpace: hasText(existing.compromiseSpace?.content)
            ? existing.compromiseSpace
            : fallback.compromiseSpace,
        }
      }
      return mergeTermSectionCommentsIntoDetail(base, item.sectionComments)
    })()
    : null

  function termSectionStorageKey(sectionKey: TermSectionCommentKey) {
    return selectedId ? `${selectedId}|${sectionKey}` : ""
  }

  function displayTermSectionComments(
    base: { author: string; content: string; time: string }[],
    sectionKey: TermSectionCommentKey,
  ) {
    const k = termSectionStorageKey(sectionKey)
    return [...base, ...(k ? extraSectionComments[k] ?? [] : [])]
  }

  async function submitTermSectionComment(sectionKey: TermSectionCommentKey) {
    const storageKey = termSectionStorageKey(sectionKey)
    if (!storageKey || !selectedId) return
    const text = (termSectionCommentInputs[storageKey] ?? "").trim()
    if (!text) return
    if (isPersistedProject && projectId && onAddTermSectionComment) {
      try {
        await onAddTermSectionComment({
          projectId,
          termId: selectedId,
          sectionKey,
          content: text,
        })
        setTermSectionCommentInputs((prev) => ({ ...prev, [storageKey]: "" }))
      } catch {
        /* 保留输入 */
      }
      return
    ...
    setExtraSectionComments((prev) => ({
      ...prev,
      [storageKey]: [...(prev[storageKey] ?? []), { author: "张伟", content: text, time: timeStr }],
    }))
    setTermSectionCommentInputs((prev) => ({ ...prev, [storageKey]: "" }))
  }
  ...
  const renderSection = (
    ...
    const storageKey = termSectionStorageKey(sectionKey)
    const mergedComments = displayTermSectionComments(section.comments, sectionKey)
    const inputVal = storageKey ? termSectionCommentInputs[storageKey] ?? "" : ""
    return (
      <div className="mb-6">
        ...
        <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
          <div className={cn("border-l-4 p-5", borderColor)}>
            ...
            <div className="p-3 bg-[#F9FAFB] rounded-lg mb-4">
              <p className="text-sm text-[#374151] leading-relaxed whitespace-pre-wrap">{section.content}</p>
            </div>
              ...
              <div className="flex items-center gap-2 mt-3">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setTermSectionCommentInputs((prev) => ({ ...prev, [storageKey]: e.target.value }))
                  }}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") void submitTermSectionComment(sectionKey)
                  }}
                />
                <button
                  type="button"
                  className="p-2 text-[#2563EB] hover:bg-[#EFF6FF] rounded-lg transition-colors disabled:opacity-40"
                  disabled={!storageKey}
                  onClick={() => void submitTermSectionComment(sectionKey)}
                >
        ...
        <div className="mx-auto max-w-5xl px-6 py-6">
          ...
          <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 mb-6">
            ...
            <p className="text-sm text-[#6B7280] mb-3">
              ID: {selectedDetail.termId} | 创建时间: {formatDisplayDateTime(selectedDetail.createdAt)} | 更新时间: {formatDisplayDateTime(selectedDetail.updatedAt)}
            </p>
            <div className="flex items-center gap-2">
            ...
            {selectedDetail.negotiationResult.content ? (
              <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
                <div className="border-l-4 border-[#0EA5E9] p-5">
                  ...
                  <div>
                    <p className="text-xs text-[#6B7280] mb-2">评论</p>
                    {displayTermSectionComments(selectedDetail.negotiationResult.comments, "negotiationResult").map((c, idx) => (
                      <div key={idx} className="flex items-start gap-2 mb-2">
                        <div className="h-6 w-6 rounded-full bg-[#E5E7EB] flex items-center justify-center shrink-0">
                          <span className="text-[10px] text-[#6B7280]">{c.author.slice(0, 1)}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        ...
                        placeholder="添加评论..."
                        className="flex-1 text-sm border border-[#E5E7EB] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB]"
                        value={termSectionCommentInputs[termSectionStorageKey("negotiationResult")] ?? ""}
                        onChange={(e) => {
                          const k = termSectionStorageKey("negotiationResult")
                          if (!k) return
                          setTermSectionCommentInputs((prev) => ({ ...prev, [k]: e.target.value }))
                        }}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") void submitTermSectionComment("negotiationResult")
                        }}
                      />
                      <button
                        type="button"
                        className="p-2 text-[#2563EB] hover:bg-[#EFF6FF] rounded-lg transition-colors"
                        onClick={() => void submitTermSectionComment("negotiationResult")}
                      >
                        <Send className="h-4 w-4" />
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        type="text"
                        placeholder="添加评论..."
                        className="flex-1 text-sm border border-[#E5E7EB] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0EA5E9]/20 focus:border-[#0EA5E9]"
                        value={termSectionCommentInputs[termSectionStorageKey("negotiationResult")] ?? ""}
                        onChange={(e) => {
                          const k = termSectionStorageKey("negotiationResult")
                          if (!k) return
                      ...
                      <button
                        type="button"
                        className="p-2 text-[#0EA5E9] hover:bg-[#F0F9FF] rounded-lg transition-colors"
                        onClick={() => void submitTermSectionComment("negotiationResult")}
                      >
                        <Send className="h-4 w-4" />
                      </button>
            ...
            {selectedDetail.implementationStatus.content ? (
              <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
                <div className="border-l-4 border-[#10B981] p-5">
                  ...
                  <div>
                    <p className="text-xs text-[#6B7280] mb-2">评论</p>
                    {displayTermSectionComments(selectedDetail.implementationStatus.comments, "implementationStatus").map((c, idx) => (
                      <div key={idx} className="flex items-start gap-2 mb-2">
                        <div className="h-6 w-6 rounded-full bg-[#E5E7EB] flex items-center justify-center shrink-0">
                          <span className="text-[10px] text-[#6B7280]">{c.author.slice(0, 1)}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <input
                        type="text"
                        placeholder="添加评论..."
                        className="flex-1 text-sm border border-[#E5E7EB] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#10B981]/20 focus:border-[#10B981]"
                        value={termSectionCommentInputs[termSectionStorageKey("implementationStatus")] ?? ""}
                        onChange={(e) => {
                          const k = termSectionStorageKey("implementationStatus")
                          if (!k) return
                          setTermSectionCommentInputs((prev) => ({ ...prev, [k]: e.target.value }))
                        }}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") void submitTermSectionComment("implementationStatus")
                        }}
                      />
                      <button
                        type="button"
                        className="p-2 text-[#10B981] hover:bg-[#ECFDF5] rounded-lg transition-colors"
                        onClick={() => void submitTermSectionComment("implementationStatus")}
                      >
                        <Send className="h-4 w-4" />
                      </button>
                    </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className={cn(
                ...
                activeDirection === "all" ? "bg-[#1E3A5F] text-white" : "bg-[#F3F4F6] text-[#4B5563] hover:bg-[#E5E7EB]"
              )}
            >
              全部目录
            </button>
            {directionTabs.map((direction) => (
              <button
                ...
                className={cn(
                  "rounded-lg px-3 py-1.5 text-xs font-medium transition-colors",
                  activeDirection === direction ? "bg-[#2563EB] text-white" : "bg-[#F8FAFC] text-[#475569] hover:bg-[#E2E8F0]"
                )}
              >
                {direction}
              </button>
            ...
            <tbody>
              {filteredData.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 max-w-xl">
                    ...
                    {item.body ? (
                      <div className="mt-2">
                        {(() => {
                          const sections = buildDisplayTermSections(item.body)
                          const isExpanded = expandedBodies[item.id] === true
                          const valueSections = sections.filter((s) => s.kind === "value")
                          const riskSections = sections.filter((s) => s.kind === "risk")
                          const opinionSections = sections.filter((s) => s.kind === "opinion")
                          const supportSections = sections.filter((s) => s.kind === "support")
                          const extraSections = sections.filter((s) => s.kind === "other")
                          const focus = defaultTermBodyFocus(bodyFocus[item.id], {
                            value: valueSections,
                            risk: riskSections,
                              ...
                              : focus === "risk"
                                ...
                                : focus === "opinion"
                                  ...
                                  : focus === "support"
                                    ? supportSections
                                    : extraSections
                          const previewSections = previewPool
                          const previewTone =
                            focus === "value"
                              ? { wrap: "border-l-4 border-[#16A34A] bg-[#F0FDF4]", title: "text-[#166534]", body: "text-[#166534]" }
                              : focus === "risk"
                                ? { wrap: "border-l-4 border-[#DC2626] bg-[#FEF2F2]", title: "text-[#991B1B]", body: "text-[#991B1B]" }
                                : focus === "opinion"
                                  ? { wrap: "border-l-4 border-[#2563EB] bg-[#EFF6FF]", title: "text-[#1D4ED8]", body: "text-[#1E40AF]" }
                                  : focus === "support"
                                    ? { wrap: "border-l-4 border-[#D97706] bg-[#FFFBEB]", title: "text-[#B45309]", body: "text-[#92400E]" }
                                    : { wrap: "border-l-4 border-[#9CA3AF] bg-[#F9FAFB]", title: "text-[#374151]", body: "text-[#4B5563]" }
                          return (
                            <div className="rounded border border-[#E5E7EB] bg-[#FAFAFA]">
                              <button
                                type="button"
                                onClick={() => setExpandedBodies((prev) => ({ ...prev, [item.id]: !isExpanded }))}
                                className="flex w-full items-center justify-between px-2 py-1.5 text-left hover:bg-[#F3F4F6]"
                              >
                                <div className="flex flex-wrap items-center gap-1.5">
                                  <span className="text-[11px] text-[#4B5563]">点击{isExpanded ? "收起" : "展开"}条款细则</span>
                                  <button
                                    type="button"
                                    className="rounded bg-[#F0FDF4] px-1.5 py-0.5 text-[10px] font-medium text-[#15803D]"
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      setExpandedBodies((prev) => ({ ...prev, [item.id]: false }))
                                      setBodyFocus((prev) => ({ ...prev, [item.id]: "value" }))
                                    }}
                                  >
                                    价值 {valueSections.length}
                                  </button>
                                  <button
                                    type="button"
                                    className="rounded bg-[#FEF2F2] px-1.5 py-0.5 text-[10px] font-medium text-[#B91C1C]"
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      setExpandedBodies((prev) => ({ ...prev, [item.id]: false }))
                                      setBodyFocus((prev) => ({ ...prev, [item.id]: "risk" }))
                                    }}
                                  >
                                    风险 {riskSections.length}
                                  </button>
                                  <button
                                    type="button"
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      setExpandedBodies((prev) => ({ ...prev, [item.id]: false }))
                                      setBodyFocus((prev) => ({ ...prev, [item.id]: "opinion" }))
                                    }}
                                  >
                                    评论 {opinionSections.length}
                                  </button>
                                  <button
                                    type="button"
                                    className="rounded bg-[#FFFBEB] px-1.5 py-0.5 text-[10px] font-medium text-[#B45309]"
                                    onClick={(e) => {
                                      e.stopPropagation()
                                      setExpandedBodies((prev) => ({ ...prev, [item.id]: false }))
                                      setBodyFocus((prev) => ({ ...prev, [item.id]: "support" }))
                                    }}
                                  >
                                    依据 {supportSections.length}
                                  </button>
                                  {extraSections.length > 0 ? (
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        setExpandedBodies((prev) => ({ ...prev, [item.id]: false }))
                                        setBodyFocus((prev) => ({ ...prev, [item.id]: "extra" }))
                                      }}
                                    >
                                      条款细则 {extraSections.length}
                                    </button>
                                  ) : null}
                                </div>
                                {isExpanded ? <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[#6B7280]" /> : <ChevronRight className="h-3.5 w-3.5 shrink-0 text-[#6B7280]" />}
                              </button>
                              {!isExpanded && previewSections.length > 0 ? (
                                <div className="max-h-[min(24rem,50vh)] space-y-2 overflow-y-auto border-t border-[#E5E7EB] p-2">
                                  {previewSections.map((section, idx) => (
                                    <div key={`${item.id}-preview-${focus}-${idx}`} className={cn("rounded p-2", previewTone.wrap)}>
                                      <p className={cn("text-[11px] font-semibold", previewTone.title)}>{section.displayTitle}</p>
                                      <p className={cn("mt-1 text-xs leading-relaxed whitespace-pre-wrap", previewTone.body)}>{section.content}</p>
                                    </div>
                                  ))}
                                </div>
                              ) : null}
                              {!isExpanded && previewSections.length === 0 ? (
                                <div className="border-t border-[#E5E7EB] px-2 py-2 text-[11px] text-[#9CA3AF]">当前维度暂无分段内容，可展开查看全部或切换上方标签。</div>
                              ) : null}
                              {isExpanded ? (
                                <div className="space-y-2 border-t border-[#E5E7EB] p-2">
                                  {valueSections.map((section, idx) => (
                                    <div key={`${item.id}-value-${idx}`} className="rounded border-l-4 border-[#16A34A] bg-[#F0FDF4] p-2">
                                      <p className="text-[11px] font-semibold text-[#166534]">{section.displayTitle}</p>
                                      <p className="mt-1 text-xs leading-relaxed text-[#166534] whitespace-pre-wrap">{section.content}</p>
                                    </div>
                                  ))}
                                  {riskSections.map((section, idx) => (
                                    <div key={`${item.id}-risk-${idx}`} className="rounded border-l-4 border-[#DC2626] bg-[#FEF2F2] p-2">
                                      <p className="text-[11px] font-semibold text-[#991B1B]">{section.displayTitle}</p>
                                      <p className="mt-1 text-xs leading-relaxed text-[#991B1B] whitespace-pre-wrap">{section.content}</p>
                                    </div>
                                  ))}
                                  {opinionSections.map((section, idx) => (
                                    <div key={`${item.id}-opinion-${idx}`} className="rounded border-l-4 border-[#2563EB] bg-[#EFF6FF] p-2">
                                      <p className="text-[11px] font-semibold text-[#1D4ED8]">{section.displayTitle}</p>
                                      <p className="mt-1 text-xs leading-relaxed text-[#1E40AF] whitespace-pre-wrap">{section.content}</p>
                                    </div>
                                  ))}
                                  {supportSections.map((section, idx) => (
                                    <div key={`${item.id}-support-${idx}`} className="rounded border-l-4 border-[#D97706] bg-[#FFFBEB] p-2">
                                      <p className="text-[11px] font-semibold text-[#B45309]">{section.displayTitle}</p>
                                      <p className="mt-1 text-xs leading-relaxed text-[#92400E] whitespace-pre-wrap">{section.content}</p>
                                    </div>
                                  ))}
                                  {extraSections.map((section, idx) => (
                                    <div key={`${item.id}-extra-${idx}`} className="rounded border-l-4 border-[#9CA3AF] bg-[#F9FAFB] p-2">
                                      <p className="text-[11px] font-semibold text-[#374151]">{section.displayTitle}</p>
                                      <p className="mt-1 text-xs leading-relaxed text-[#4B5563] whitespace-pre-wrap">{section.content}</p>
                                    </div>
                                  ))}
                                </div>
                              ) : null}
                  ...
                  <td className="px-4 py-3 text-sm text-[#6B7280] whitespace-nowrap">{formatDisplayDateTime(item.createdAt)}</td>
                  <td className="px-4 py-3 text-sm text-[#6B7280] whitespace-nowrap">{formatDisplayDateTime(item.updatedAt)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
              <div className="flex items-center justify-between border-b border-[#E5E7EB] px-6 py-4">
                <h3 className="text-base font-semibold text-[#111827]">新建条款</h3>
                <button onClick={() => setShowCreateDialog(false)} className="rounded p-1 text-[#6B7280] hover:bg-[#F3F4F6]">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="space-y-4 px-6 py-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-[#374151]">条款方向</label>
                    <Input value={createForm.direction} onChange={(e) => setCreateForm((prev) => ({ ...prev, direction: e.target.value }))} placeholder="例如：一、资本安全与下行保护" />
                  </div>
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-[#374151]">条款类别</label>
                    <Input value={createForm.category} onChange={(e) => setCreateForm((prev) => ({ ...prev, category: e.target.value }))} placeholder="例如：优先清偿权" />
                  </div>
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#374151]">条款名称</label>
                  <Input value={createForm.name} onChange={(e) => setCreateForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="请输入条款名称" />
                </div>
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#374151]">正文（可选）</label>
                  <textarea
                    rows={6}
                    value={createForm.body}
                    onChange={(e) => setCreateForm((prev) => ({ ...prev, body: e.target.value }))}
                    className="w-full rounded-lg border border-[#E5E7EB] px-3 py-2 text-sm text-[#111827] focus:border-[#2563EB] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20"
                    placeholder="支持按【标题】格式录入，前端会自动结构化展示"
                  />
                </div>
                {createError ? <p className="text-sm text-[#DC2626]">{createError}</p> : null}
              </div>
              <div className="flex items-center justify-end gap-3 border-t border-[#E5E7EB] px-6 py-4">
                <Button variant="outline" onClick={() => setShowCreateDialog(false)} disabled={isCreating}>取消</Button>
                <Button onClick={handleCreateTermSubmit} disabled={isCreating}>
                  {isCreating ? "创建中..." : "确认创建"}
                </Button>
              </div>
            </div>
